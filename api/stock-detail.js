module.exports = async function handler(req, res) {
  try {
    ...
    const historyUrl =
      `${baseUrl}/api/market-history?${historyParams.toString()}`;
    ...
    if (
      newestFirst.length < 60
    ) {
      return res.status(422).json({
        ok: false,
        error:
          "점수 계산에 필요한 데이터가 부족합니다.",
        collectedDays:
          newestFirst.length
      });
    }
